...
exports.register = async (req, res, next) => {
  ...
  try {
    ...
    return res
      .status(201)
      .json({ message: "User created!", userId: result._id }); // returning user id as result.
  } catch (err) {
...
exports.login = async (req, res, next) => {
  ...
  try {
    ...
    // res.cookie('jwt', token, {maxAge: 14 * 60 * 60 * 1000}) ORIGINAL
...
exports.updateCarbonFootprint = async (req, res, next) => {
  try {
    ...
    if (!user) {
      ...
      return res
        .status(200)
        .json({ Message: "success", newValue: user.carbonFootprint }); // Returning the users to the frontend.
    }
  } catch (err) {
    if (!err.statusCode) {
      // Returning any other errors.
...
function addtotalTree(totalCarbonInfo) {
  const totalTree = country_controller.carbonToTrees(
    totalCarbonInfo.totalCarbonOffset
  );

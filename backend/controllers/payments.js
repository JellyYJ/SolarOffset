...
const YOUR_DOMAIN = process.env.FRONT_END_HOST + "/countries";
...
exports.validatePaymentStatus = async (req, res, next) => {
    ...
    try {
        ...
        if (session.payment_status === 'paid') {
            ...
            if (userId) {
                await userController.updateUserData(email, session.amount_total / 100.0, result.carbonOffset, numPanels);
            }
            logger.info({message: 'Payment saved!', paymentId: result._id});
            return res.status(201).json({message: 'Payment saved!', payment: result});
        } else {
            return res.status(500).json({payment_status: session.payment_status, status: session.status});
        }
    } catch (err) {
        // Returning any other errors.
        if (!err.statusCode) {
...
exports.getAllPayments = async (req, res, next) => {
    try {
        ...
        logger.info('User retrieved all payments.', {userId: req.userId, role: req.role}) // Logging user who retrieved all users.
...
exports.getPaymentsByEmail = async (req, res, next) => {
    ...
    try {
        ...
        logger.info('User retrieved payments by email.', {userId: req.userId, role: req.role, email: userEmail}) // Logging user who retrieved all users.
...
exports.getPaymentByEmail = async (userEmail) => {
    try {
        ...
        throw new Error(`Failed to get payments for email ${userEmail}: ${err.message}`);
    }
};
